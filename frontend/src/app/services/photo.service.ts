...
export class PhotoService {
  ...
  getPhotos() {
    let url = "https://whispering-coast-92817.herokuapp.com/photo/allPhotos";
    return this.http.get(url);
  }

  getPhotoById (photoId: number) {
    let tokenUrl1 = "https://whispering-coast-92817.herokuapp.com/rest/photo/photoId";
  ...
  getPhotosByUser (user: User) {
    let tokenUrl1 = "https://whispering-coast-92817.herokuapp.com/rest/photo/user";
    let headers1 = new Headers({'Content-Type': 'application/json', 'Authorization': 'Bearer '+localStorage.getItem("token")});
    return this.http.post(tokenUrl1, JSON.stringify(user), {headers: headers1});
  }

  updatePhoto(photo: Photo) {
    let tokenUrl1 = "https://whispering-coast-92817.herokuapp.com/rest/photo/update";
    let headers1 = new Headers({'Content-Type': 'application/json', 'Authorization': 'Bearer '+localStorage.getItem("token")});
